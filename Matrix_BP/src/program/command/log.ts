...
new Command()
	...
	.onExecute(async (player) => {
		const ui = new ActionFormData()
			.title(rawtextTranslate("command.logui.title"));
		const restartLogs = log.getRestartLogs()
		restartLogs.forEach((log) => {
			ui.button(fastText().addTran("command.logui.button", log.amount).endline().addText("§8" + generateShortTimeStr(log.now)).build());
		});
